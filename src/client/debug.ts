// MODISA_DEBUG_MOUSE=1: OpenTUI logs raw terminal input (its OTUI_STDIN_LOG) and we log what the
...
export async function openMouseLog(): Promise<(line: string) => void> {
  if (!Bun.env.MODISA_DEBUG_MOUSE) return () => {};
...
export function logHandlerErrors(r: CliRenderer, debug: (line: string) => void) {
  if (Bun.env.MODISA_DEBUG_MOUSE) r.on("handler:error", ({ error, event }: any) => debug(`opentui handler error on ${event?.type}: ${error}`));
}
