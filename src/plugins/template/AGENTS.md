# Writing the {{name}} modisa plugin
...
modisa plugin logs {{name}}    # its stdout and stderr
modisa plugin run {{name}} status '{"any":"params"}'
modisa plugin stop {{name}}    # and plugin start {{name}}
```
