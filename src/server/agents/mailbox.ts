...
export type Message = { id: number; from: string; fromName: string; to: string; toName: string; body: string; hops: number; at: number; delivered?: number };
...
export class Mailbox {
  ...
  send(from: string, fromName: string, to: string, toName: string, body: string): Message {
    ...
    const m: Message = { id: ++this.seq, from, fromName, to, toName, body, hops, at: Date.now() };
    this.log.push(m);
  ...
  static frame(m: Message) {
    ...
    const reply = m.from === "user" ? "" : ` (reply: shepherd send @${m.fromName} "...")`;
