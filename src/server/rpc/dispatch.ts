...
export function createDispatcher(handlers: Handlers) {
  return async function dispatch(c: Client, m: Msg) {
    ...
    if (!h) return reply({ error: { code: -32601, message: `unknown method ${m.method}` } });
    ...
    if (!parsed.success) return reply({ error: { code: -32602, message: `invalid params: ${parsed.error.issues.map((i) => `${i.path.join(".")} ${i.message}`).join("; ")}` } });
    try {
      reply({ result: (await h(parsed.data, c)) ?? null });
    } catch (e: any) {
      reply({ error: { code: -32000, message: e?.message ?? String(e) } });
    }
  };
}
