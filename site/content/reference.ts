...
export const reference: Page[] = [
  {
    ...
    description: "Which agents modisa knows, the states it tracks, and how it reads them.",
    sections: [
      { id: "known", title: `${agents.length} agents, no setup`, html: p(`Modisa recognises these by their process, including when they're started through node, bun, python or a shell script:`) +
      ...
      { id: "custom", title: "Add or tune an agent", html: p(`A TOML file in ${c("~/.config/modisa/adapters/<id>.toml")} adds an agent or overrides a built-in one:`) + code("toml", `name = "My Agent"
process = ["my-agent"]
launch = "my-agent"
...
regex = ['(?i)esc to interrupt']`) + p(`${c("modisa debug detect <pane>")} shows which rule matched, whether an integration is reporting, and the title and progress it saw.`) },
    ],
  },
  {
    ...
    sections: [
      ...
      { id: "install", title: "Install", html: code("sh", `modisa integration status          # installed, update available, available, not found
modisa integration install all     # everything recommended for the agents on this machine
modisa integration install codex   # or one
modisa integration uninstall codex # takes out only modisa's entries`) + p(`Or ${kbd("Ctrl+B")} ${kbd("s")} → integrations. Each one goes into the agent's own config, in its own format, next to your settings; an agent's config directory has to exist (run it once). Installs happen where the server runs, which is where the agents are.`) },
      { id: "yours", title: "Report from your own tools", html: code("sh", `modisa report --source my-tool --agent my-agent --state working
modisa report --source my-tool --state blocked --seq 42   # stale seqs are ignored
modisa report --source my-tool --session-id abc123        # resume this session after a restart
modisa report --source my-tool --release                  # hand the pane back to screen detection`) + p(`Run it inside a pane (it reads ${c("MODISA_PANE_ID")}) or name the pane first. Reports without ${c("--source")} don't change state.`) },
    ],
  },
  {
    ...
    sections: [
      { id: "send", title: "Send and read", html: code("sh", `modisa send @reviewer "fixed, please re-check"   # typed into the agent once it's idle
modisa inbox                                     # messages sent to you
modisa messages --follow                          # every message, live`) + p(`A message waits until its recipient is idle, then is typed in with a reply hint naming the sender pane, like ${c('modisa send p3:1a2b3c4d "..."')}. The same hint is in ${c("modisa inbox")}. That target survives a rename, but not a server restart: it fails, rather than reaching another pane, once the sender has closed or the server has restarted.`) },
  ...
  },
  {
    ...
    sections: [
      { id: "env", title: "From inside a pane", html: p(`Every pane gets ${c("MODISA_SOCKET")} and ${c("MODISA_PANE_ID")}, so anything that can run a command can drive the workspace — and targets default to the calling pane.`) + code("sh", `modisa pane split --name tests "pnpm test"     # opens next to the caller
modisa wait tests --exited                     # prints "exited <code>"
modisa pane read tests --lines 80
modisa agent spawn codex --name reviewer --prompt "review src/auth"
...
modisa events --follow`) },
  ...
  },
  {
    ...
    sections: [
      { id: "what", title: "What it is", html: p(`Installing an integration also drops in the modisa skill: a ${c("SKILL.md")} covering splitting panes, running commands in them, reading output, spawning and messaging other agents, and waiting on them. The agent loads it only when the task calls for it, so it costs nothing the rest of the time, and it works through the same ${c("modisa")} binary that is already on every pane's PATH — there is no server to run and no tool slots to spend.`) },
      { id: "install", title: "Install it", html: p(`There's no separate command — the skill comes with the integration:`) + code("sh", `modisa integration install claude     # hooks + the skill, for one agent
modisa integration install all        # …for every agent on this machine
modisa integration status             # "↻ update available" when a newer skill ships
modisa integration uninstall claude   # takes the skill back out too`) + p(`Restart a running agent to pick it up.`) },
      { id: "where", title: "Where it goes", html: code("text", `~/.agents/skills/modisa/SKILL.md          the one copy
~/.claude/skills/modisa  -> …/.agents/skills/modisa
~/.codex/skills/modisa   -> …/.agents/skills/modisa`) + p(`One copy, symlinked into each agent that reads skills from a directory of its own. Every agent with an integration gets it except MastraCode and OMP, whose skills directories aren't established; Kimi Code reads ${c("~/.agents/skills")} itself, so it needs no link.`) },
    ],
  },
  {
    ...
    sections: [
      { id: "write", title: "Write one", html: code("sh", `modisa plugin new my-plugin     # plugin.json, plugin.ts, a test, the client library, AGENTS.md
...
modisa plugin unlink attention-log`) + p(
...
modisa plugin search github     # …that also matches these words`) + p(
  ...
  { "regex": "^https://github\\\\.com/[^/]+/[^/]+/pull/\\\\d+$", "action": "open-pr" }
      ...
      { id: "not-a-pane", title: "A plugin is not a pane", html: p(
        `${c("MODISA_PANE_ID")} is deliberately unset, so commands that default to "the calling pane" have no default — always pass a ${c("target")}. And you act with the <strong>user's</strong> authority: the permission prompts that gate an agent typing into or closing a pane it didn't create do not apply to you.`,
      ) },
      ...
      { id: "shell", title: "Without a manifest", html: code("toml", `[[plugin]]\nrun = "bun ~/code/watcher/plugin.ts"`) + p(
        `A ${c("[[plugin]]")} line in config.toml starts any program with the session and gives it ${c("MODISA_SOCKET")}, which is the whole API. ${c("run")} goes through a <strong>login</strong> shell, so your profile can rewrite ${c("PATH")}: use absolute paths. Or skip the socket client — the CLI reads the same variable:`,
      ) + code("sh", `#!/bin/sh
modisa events --follow | while read -r line; do
  ...
  },
  {
    ...
    sections: [
      { id: "where", title: "Where it lives", html: p(`${c("~/.config/modisa/config.toml")}. The settings page edits it for you; ${c("modisa config edit")} opens it in your editor. Changes apply live.`) },
  ...
  },
  {
    ...
    sections: [
      { id: "badge", title: "The update badge", html: p(`When a newer release is out, the status row shows ${c("↑ 0.x.y")}. Click it (or run "Update modisa" from the palette) to download it and restart the server; your agents come back where they were. Modisa checks at most every six hours.`) },
      { id: "cli", title: "From the command line", html: code("sh", `modisa version     # this version, and whether a newer one is out
modisa update      # download, verify its SHA-256, replace this binary
modisa restart     # load it into running sessions`) },
      { id: "managed", title: "Installed with a package manager", html: p(`A package manager owns the binary it installed, so ${c("modisa update")} and the update badge give you that tool's command instead (${c("mise upgrade github:manyeya/modisa")}, or ${c("apt")} or ${c("dnf")} for the .deb and .rpm packages). Run ${c("modisa restart")} afterwards. Agents' hooks call the ${c("modisa")} on your ${c("PATH")}, so they keep working across upgrades.`) },
      { id: "verify", title: "Verifying a release", html: p(`Every release file has signed build provenance. ${c("gh attestation verify <file> -R manyeya/modisa")} confirms that this repository's release workflow built the file from a tagged commit.`) },
      { id: "channels", title: "Channels", html: p(`${c("stable")} follows releases. ${c("staging")} follows prereleases built from the staging branch. Set it in ${c("[update] channel")}, or install from it with ${c("MODISA_CHANNEL=staging")}. ${c("[update] check = false")} turns the check off.`) },
    ],
  },
  {
    ...
    sections: [
      { id: "state", title: "An agent shows the wrong state", html: p(`Run ${c("modisa debug detect <pane>")} to see the rule that matched and the screen it read. A new agent version may draw something the rules don't know yet: override the agent's rules in ${c("~/.config/modisa/adapters/")}.`) },
      { id: "stale", title: "Something changed but nothing's different", html: p(`A running session keeps the modisa it started with. ${c("modisa restart")} loads the new one (agents resume), then detach and reattach to refresh the client too.`) },
      { id: "logs", title: "Logs", html: p(`Each session's server logs to ${c("~/.local/state/modisa/<session>.log")}.`) },
      { id: "outdated", title: "An integration says “update available”", html: p(`It was installed by an older modisa, or modisa has moved. ${c("modisa integration install all")} brings every one up to date.`) },
      { id: "sandbox", title: "An agent in a sandbox can't reach modisa", html:
        p(`Some agents run their commands in a sandbox that blocks modisa's socket. Their ${c("modisa")} commands then fail with “can't connect to its socket: a sandbox is blocking it”. You can still message that agent, but it can't ${c("send")}, read panes or spawn agents.`) +
        ...
        code("toml", `default_permissions = "modisa"

[permissions.modisa]
extends = ":workspace"

[permissions.modisa.network]
enabled = true`) +
      ...
      { id: "sound", title: "No sound", html: p("Check the sound section of the settings page and your output device. Sounds are generated and played by modisa itself, so nothing else needs installing.") },
    ],
  },
];
