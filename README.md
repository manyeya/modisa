...
```bash
curl -fsSL https://manyeya.github.io/modisa/install.sh | sh
```

Releases are built for macOS on Apple silicon and Linux on x64 and arm64. The installer checks the download against its published SHA-256 and puts `modisa` in `~/.local/bin` (`MODISA_INSTALL_DIR` changes that; `MODISA_CHANNEL=staging` installs the latest prerelease).

Or install with a package manager, which then updates and removes it:

| With | Install | Remove |
|------|---------|--------|
| mise | `mise use -g github:manyeya/modisa` | `mise uninstall github:manyeya/modisa` |
| Debian, Ubuntu | `sudo apt install ./modisa_<version>_<arch>.deb` | `sudo apt remove modisa` |
| Fedora, RHEL | `sudo dnf install ./modisa-<version>-1.<arch>.rpm` | `sudo dnf remove modisa` |

Every release on [GitHub Releases](https://github.com/manyeya/modisa/releases) has the binaries, the .deb and .rpm packages, `SHA256SUMS`, the `manifest.json` the installer and updater read, and signed build provenance. To check that a file was built by this repository's release workflow from a tagged commit, run `gh attestation verify modisa-darwin-arm64 -R manyeya/modisa`.

When a newer release is out, the status row shows `↑ <version>`: click it to update and restart the server (agents resume). From the command line: `modisa version`, `modisa update`, then `modisa restart`. If mise or a package installed modisa, update with that tool instead (modisa tells you the command). `[update] channel = "staging"` follows prereleases; `[update] check = false` turns the check off.

### Uninstall

```bash
modisa uninstall        # or: curl -fsSL https://manyeya.github.io/modisa/install.sh | sh -s -- --uninstall
```
...
Spaces are named groups of tabs and panes. Each one shows where its focused pane's git repository stands: the branch, ↑ commits to push, ↓ to pull (as of your last fetch) and ● files changed (`[sidebar] git = false` hides it). Agents show their logo in their brand colour and the task they're on: the logos are a small font modisa installs the first time it starts, for Ghostty, kitty and VS Code (restart the terminal once; `modisa logos` shows where they stand, `modisa logos uninstall` takes them out). In the sidebar, click a space to switch to it, double-click its name (or click ✎) to rename it in place — Enter saves, Esc cancels — and click ✕ to delete it, which asks first and closes its panes. Right-click a space for the same actions. The last space can't be deleted. A new space starts in the current space's working directory; creating one does not create a directory. Use the shell's `cd` command to change a pane's working directory.

The settings page (`Ctrl+B s`, or ⚙ settings in the sidebar) has a tab per section; Tab switches section, ↑↓ or the pointer selects, ←→ changes a value, Enter or a click applies, Esc closes. Every change applies at once and is saved to `~/.config/modisa/config.toml`, keeping your comments and other settings.
...
```bash
modisa                    # attach the default session (starts it if needed)
modisa new api --cwd ~/code/api
...
`--remote` splits modisa in two: the TUI runs on your machine, the server and every pane run on the far side. Your keybindings, theme, sounds and notifications stay local; the panes, the agents and their files are remote.

```bash
modisa --remote ssh://devbox              # host
modisa --remote ssh://me@build-01:2222    # user and port
modisa --remote devbox                    # a Host alias from ~/.ssh/config
modisa -s api --remote ssh://devbox       # a named session on that machine
```

There is no daemon to install and no port to open. Modisa shells out to your own `ssh`, so agent forwarding, jump hosts, `Match` blocks and keys from `~/.ssh/config` all apply. The exact command it runs:

```bash
ssh -T devbox modisa proxy -s default             # ssh://devbox
ssh -T -p 2222 me@build-01 modisa proxy -s api    # ssh://me@build-01:2222, -s api
```

`modisa proxy` on the far side starts the session server if it isn't running, then bridges its unix socket to stdio. `-T` means no pty is allocated: the protocol is newline-delimited JSON, not a terminal.

**Modisa must be on the remote machine**, and by default it must be on the `PATH` of a
*non-interactive* ssh shell — which often excludes `~/.local/bin`, where the installer puts it. If
`--remote` fails with something like `modisa: command not found`, set an absolute path in your
**local** config (`~/.config/modisa/config.toml`):

```toml
remote_command = "/home/me/.local/bin/modisa"
```

Check what the remote shell actually sees with `ssh devbox 'command -v modisa'`.
...
- **Sessions are per machine.** `modisa ls` lists local sessions; to see the remote ones, `ssh devbox modisa ls`.
...
- **Updating is per machine.** `modisa update` on your laptop doesn't touch the remote install; run it there too, then `modisa restart`.
- `MODISA_SSH` overrides the ssh binary, which is how the test suite substitutes a fake.

A `modisa.toml` in the directory you start from lays out a new session:
...
```bash
modisa integration status              # every agent: installed, update available, available, not found
modisa integration install all         # everything recommended for the agents on this machine
modisa integration install codex       # or one agent; uninstall takes out only modisa's entries
```
...
Installing an integration also drops in the **modisa skill** — a `SKILL.md` teaching the agent to split panes, spawn and message other agents, and wait on them, so it drives the session without you explaining the CLI each time. There's no separate command; it comes with the integration:

```bash
modisa integration install claude       # hooks + the skill, for one agent
...
modisa integration uninstall claude     # takes the skill back out too
```

```text
~/.agents/skills/modisa/SKILL.md          the one copy
~/.claude/skills/modisa  -> …/.agents/skills/modisa
~/.codex/skills/modisa   -> …/.agents/skills/modisa
```
...
Your own tools can report too: `modisa report --source my-tool --agent my-agent --state working|blocked|idle [--seq n] [--session-id id]`, and `--release` hands the pane back to screen detection. Reports without `--source` don't change state.

Add an agent or override one with a TOML file in `~/.config/modisa/adapters/<id>.toml` (`name`, `process`, `launch`, `resume`, `resumeSession`, and `[[rules]]` in the manifest format). `modisa debug detect <pane>` shows the matched rule, the reporting integration, and the title and progress it saw.

## Driving modisa from an agent

Every pane gets `MODISA_SOCKET` and `MODISA_PANE_ID`, so any agent that can run shell commands can drive the workspace:

```bash
modisa pane split --name tests "pnpm test"     # opens next to the caller
...
modisa send @reviewer "fixed, please re-check"  # typed into the agent once it's idle
modisa inbox
...
A plugin is a program modisa starts with each session, connected to the session's socket: it watches what happens, offers actions, and can put a little into the TUI. `modisa plugin new` scaffolds one in TypeScript with modisa's client library, a behavioural test, and `AGENTS.md`, a guide an agent can follow to write the rest.

```sh
modisa plugin new my-plugin          # plugin.json, plugin.ts, a test, the client library, AGENTS.md
...
`Ctrl+B s` opens the settings page; `modisa config edit` (or "Edit config.toml" in the command palette) opens `~/.config/modisa/config.toml` itself; changes apply live. It covers the prefix key, theme, sidebar, which notifications fire (toast, system, sound, bell), messaging limits, permissions, per-agent launch commands, `remote_command` for `--remote`, `[plugin_keys]`, and `[[plugin]]` programs.
