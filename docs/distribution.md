...
Goal: modisa is easy to install, easy to upgrade, easy to remove, and anyone can check that a binary came from this repository.
...
- [x] `modisa uninstall [--purge] [--yes]`: integrations, the shared skill, sessions, state, and config with `--purge`; deletes the binary only when install.sh put it there
- [x] `install.sh --uninstall` hands off to `modisa uninstall`
- [x] Hooks call the `modisa` on `PATH` from a release binary, so per-version install directories (Homebrew, mise) don't break them on upgrade
- [x] `modisa update` and the update badge print the Homebrew, mise or apt/dnf command instead of replacing a binary a package manager owns (`src/core/install.ts`)
...
- [ ] Run `gh attestation verify modisa-darwin-arm64 -R manyeya/modisa` against the published file
...
- [ ] Create a fine-grained token with Contents write on that repo only, and add it as the `HOMEBREW_TAP_TOKEN` secret on `manyeya/modisa`
- [ ] Release to `main`, then confirm `Formula/modisa.rb` landed in the tap
- [ ] `brew install manyeya/tap/modisa` on a clean machine, then `modisa uninstall && brew uninstall modisa`
...
- [ ] npm: publish `@manyeya/modisa` (`modisa` is taken) with per-platform packages in `optionalDependencies` (`os`/`cpu` fields), using trusted publishing from GitHub Actions (npm CLI 11.5.1+, provenance generated automatically)
- [ ] npm: add `node_modules` to `installedBy` so updates defer to `npm update -g @manyeya/modisa`
- [ ] apt/dnf repositories on Cloudsmith's free open-source plan, so `apt upgrade` and `dnf upgrade` pick up new versions
- [ ] AUR `modisa-bin` PKGBUILD from the release binaries, regenerated each release with `updpkgsums`
...
- [ ] `stableSelf()` trusts that whatever is called `modisa` on `PATH` is this program
